...
public class danger : MonoBehaviour
{
    ...
    public void Die()//вызывается когда в астероид попали
    {
        ...
        for(int i = 0; i < 2; i++)
        {
            Instantiate(childMetior[Random.Range(0,childMetior.Length)], transform.position, Quaternion.identity);
        }
        GetComponent<Rigidbody2D>().AddExplosionForce(5, transform.position, 10f);
    }
}
